...
public class WordFilter {
	...
	private WordFilter(MongoClient mongoClient) {
	...
	public static WordFilter getInstance(MongoClient mongoClient) {
		if (wordFilter == null) {
			wordFilter = new WordFilter(mongoClient);
		}
		return wordFilter;
	}
	...
	private void init() {
		// 获取敏感词
		addSensitiveWord(WordUtils.findWord(mongoClient, "sen"));
		addStopWord(WordUtils.findWord(mongoClient, "stop"));
	}
	 ...
	 * @return
	 */
	public final String getWords(final String src) {
		char[] chs = src.toCharArray();
		int length = chs.length;
		int currc;
		int k;
		...
		StringBuffer resutBuffer = new StringBuffer();
		for (String str : result) {
			resutBuffer.append("《").append(str).append("》");
		}
		return resutBuffer.toString();
	}
